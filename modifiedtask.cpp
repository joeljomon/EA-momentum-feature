...
public:
    ...
    void remove_item(size_t item_index) {
        ...
        if (input_quantity <= items[item_index].get_quantity()) {
            float price = items[item_index].get_price();
            float money_earned = price * input_quantity;
            items[item_index].set_quantity(items[item_index].get_quantity() - input_quantity);
            total_money += money_earned;
            std::cout << "\nItems sold";
            std::cout << "\nMoney received: " << money_earned;
            
            if (items[item_index].get_quantity() == 0) {
